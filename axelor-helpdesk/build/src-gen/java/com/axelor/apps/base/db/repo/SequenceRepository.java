...
public class SequenceRepository extends JpaRepository<Sequence> {
  ...
  public Sequence findByCode(String code) {
    return Query.of(Sequence.class).filter("self.code = :code").bind("code", code).fetchOne();
  }

  public Sequence findByName(String name) {
    return Query.of(Sequence.class).filter("self.name = :name").bind("name", name).fetchOne();
  }

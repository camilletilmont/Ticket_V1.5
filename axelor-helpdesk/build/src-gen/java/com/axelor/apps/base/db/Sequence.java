...
@Table(
  name = "BASE_SEQUENCE",
  uniqueConstraints = {@UniqueConstraint(columnNames = {"company", "code", "prefixe", "suffixe"})},
  indexes = {
    @Index(columnList = "company"),
    @Index(columnList = "name"),
    @Index(columnList = "code"),
    @Index(columnList = "fullName"),
    @Index(name = "idx_sequence_code_company", columnList = "code,company")
  }
)
public class Sequence extends AuditableModel {
  ...
  @SequenceGenerator(
    ...
    sequenceName = "BASE_SEQUENCE_SEQ",
    allocationSize = 1
  )
  private Long id;
   ...
   *
   * <p>It sets {@code item.sequence = this} to ensure the proper relationship.
   *
   ...
   *
   * <p>If you have to query {@link SequenceVersion} records in same transaction, make sure to call
